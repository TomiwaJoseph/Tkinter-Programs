...
class StartPage(Frame):
    def __init__(self, root):
        ...
        # ======= Title =====================
        Label(self.master, text='Chess Clock', font='montserrat 40',
              fg='#505050', bg='#fff').place(x=150, y=83)
        Label(self.master, text='Chess Clock', font='montserrat 40',
              fg='#fff', bg='#222').place(x=150, y=80)
        # =========== Buttons ==================
        Button(self.master, text='Start', font='montserrat 18', bd=0, fg='yellow',
               bg='#222', command=lambda: self.master.switch_frame(SetTime)).place(x=280, y=150)
...
class SetTime(Frame):
    def __init__(self, root):
        ...
        self.p2_secs.set("0")
        # ============ UI =============
        Label(text="Player 1", font='poppins 16',
              bg='#222', fg='orange').place(x=90, y=58)
        Label(text='Mins:', font='montserrat 16',
              bg='#222', fg='white').place(x=230, y=58)
        self.p1_m = Entry(textvariable=self.p1_mins, width=4,
                          font='montserrat 14', justify=CENTER)
        self.p1_m.place(x=300, y=60)
        Label(text='Secs:', font='montserrat 16',
              bg='#222', fg='white').place(x=380, y=58)
        self.p1_s = Entry(textvariable=self.p1_secs, width=4,
                          font='montserrat 14', justify=CENTER)
        self.p1_s.place(x=450, y=60)

        Label(text="Player 2", font='poppins 16',
              bg='#222', fg='orange').place(x=90, y=128)
        Label(text='Mins:', font='montserrat 16',
              bg='#222', fg='white').place(x=230, y=128)
        self.p2_m = Entry(textvariable=self.p2_mins, width=4,
                          font='montserrat 14', justify=CENTER)
        self.p2_m.place(x=300, y=130)
        Label(text='Secs:', font='montserrat 16',
              bg='#222', fg='white').place(x=380, y=128)
        self.p2_s = Entry(textvariable=self.p2_secs, width=4,
                          font='montserrat 14', justify=CENTER)
    ...
    def go_without_time_set(self):
        ...
        try:
            ...
            messagebox.showerror(
                "Error", "One of the entry box is empty or an invalid value is entered in it. Please enter a number in the entry box.")
...
class ClockIt(Frame):
    def __init__(self, root):
        ...
        self.p1 = Button(command=lambda: self.know_player(
            self.p1), width=8, font='candara 48', bg='#222', fg='white')
        self.p1.place(x=30, y=60)
        self.p2 = Button(command=lambda: self.know_player(
            self.p2), width=8, font='candara 48', bg='#222', fg='white')
    ...
    def count_down_1(self):
        ...
        if self.p1_paused == True:
            if player1_minutes > -1:
                ...
                self.p1.config(text='{:02d}:{:02d}'.format(
                    mins, secs), bg='#fff', fg='green')
                player1_minutes -= 1
                self.p1.after(1000, self.count_down_1)
            else:
                self.sound_player = Thread(
                    target=playsound, args=('./static/alarm.wav',))
    ...
    def count_down_2(self):
        ...
        if self.p2_paused == True:
            if player2_minutes > -1:
                ...
                self.p2.config(text='{:02d}:{:02d}'.format(
                    mins, secs), bg='#fff', fg='green')
                player2_minutes -= 1
    ...
    def set_it(self):
        ...
        mins, secs = divmod(player1_minutes, 60)
        self.p1.config(text='{:02d}:{:02d}'.format(
            mins, secs), fg='#fff', bg='#222', state=NORMAL)
        mins, secs = divmod(player2_minutes, 60)
        self.p2.config(text='{:02d}:{:02d}'.format(
            mins, secs), fg='#fff', bg='#222', state=NORMAL)
