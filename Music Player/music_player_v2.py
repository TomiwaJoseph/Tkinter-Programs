...
class StartPage(Frame):
    def __init__(self,root):
        ...
        self.app_logo = Frame(width=62,height=60,relief='ridge',bg='#101820',bd=2)
...
class MusicPage(Frame):
    def __init__(self,root):
        ...
        self.create_ui()
        #============ Utility Frames ====================
        self.quitting_frame = Frame(width=1000,height=500,relief='ridge',bg='#101820')
    ...
    def create_keybinds(self):
        ...
        self.root.bind('<r>', self.repeat_current_song)
        self.root.bind('<space>', lambda t: self.play_song('button press') if self.original_playlist else print('Empty playlist'))
    ...
    def create_ui(self):
        ...
        self.album_art = Label(image=self.cover,relief='ridge',width=230,height=230)
        self.album_art.place(relx=0.5,rely=0.31,anchor=CENTER)
        #============ CONTROL FRAME ====================
        self.control_frame = Frame(width=850,height=80,relief='ridge',bg='#101820',bd=3)
        ...
        self.previous_btn = Button(self.control_frame,image=self.prev_img,bd=0,command=self.previous_song,bg='#101820')
        ...
        self.play_btn = Button(self.control_frame,image=self.play_img,bd=0,command=lambda:self.play_song('button press'),bg='#101820')
        ...
        self.next_btn = Button(self.control_frame,image=self.next_img,bd=0,command=lambda:self.next_song('from button'),bg='#101820')
        ...
        self.repeat_all_btn = Button(self.control_frame,image=self.view_playlist_img,bd=0,command=self.view_playlist,bg='#101820')
        ...
        self.repeat_all_btn = Button(self.control_frame,image=self.repeat_all_img,bd=0,command=self.repeat_all_songs,bg='#101820')
        ...
        self.repeat_one_btn = Button(self.control_frame,image=self.repeat_one_img,bd=0,command=self.repeat_current_song,bg='#101820')
        ...
        self.shuffle_btn = Button(self.control_frame,image=self.shuffle_img,bd=0,command=self.shuffle_playlist,bg='#101820')
        ...
        self.progress_start = Label(font=('AdobeClean-Bold', 13),fg='#f2aa4c',bg='#101820',text='5:00:00')
        self.progress_start.place(relx=0.11,rely=0.72,anchor=CENTER)
        self.progress_end = Label(font=('AdobeClean-Bold', 13),fg='#f2aa4c',bg='#101820',text='5:00:00')
        self.progress_end.place(relx=0.89,rely=0.72,anchor=CENTER)

        self.song_artist_and_title = Text(width=70,height=1,font=('AdobeClean-REGULAR', 14),fg='#101820',bg='#f2aa4c')
        self.song_artist_and_title.tag_configure('center', justify='center')
        self.song_artist_and_title.tag_configure('bold', font=('AdobeClean-Bold', 14), justify='center')
        ...
        self.speaker_btn = Button(self.control_frame,image=self.speaker_img,bd=0,command=self.mute_and_unmute_song,bg='#101820')
    ...
    def forwards_or_backwards(self, value):
        ...
        if self.paused:
            ...
            self.paused = False
        mixer.music.load(self.playlist_in_use[self.current_song_index_in_playlist])
    ...
    def shuffle_playlist(self, event=None):
        ...
        if self.shuffle:
            ...
            shuffle_false_index = [k for k, v in self.original_playlist.items() if v == self.shuffled_playlist[self.current_song_index_in_playlist]][0]
            ...
            try:
                self.shuffled_playlist = [self.original_playlist.get(i) for i in self.original_playlist.keys()]
    ...
    def get_and_set_music_title_artist(self, the_song):
        try:
            ...
            self.song_artist_and_title.insert('end', '♫   ', 'center')
            if song_title == None:
                self.song_artist_and_title.insert('end', 'Unknown Title / ', 'bold')
            else:
                self.song_artist_and_title.insert('end', song_title+ ' / ', 'bold')
    ...
    def delete_selected_playlist_item(self):
        try:
            ...
            if selection == self.current_song_index_in_playlist:
                ...
                original_playlist_key = [key for key, value in self.original_playlist.items() if value == song_to_delete][0]
                del self.original_playlist[original_playlist_key]
                # if the shuffled list has been created
                if self.shuffled_playlist != []:
                    shuffled_playlist_index = self.shuffled_playlist.index(song_to_delete)
                ...
                for i in range(len(old_keys)):
                    self.original_playlist[i] = self.original_playlist.pop(old_keys[i])
                if old_keys:
    ...
    def view_playlist(self, event=None):
        ...
        #========= UTILITY TOOLS FRAME ==================
        utility_tools_frame = Frame(self.playlistWindow,width=400,bd=2,height=53,relief='ridge',bg='#101820')
        ...
        try:
            if self.playlist_in_use == self.shuffled_playlist:
                currently_playing_index = self.shuffled_playlist.index(self.current_song)
            else:
                currently_playing_index = [k for k, v in self.original_playlist.items() if v == self.current_song][0]
            self.musicList.itemconfig(currently_playing_index, {'bg': '#f2aa4c', 'fg': '#101820'})
    ...
    def about_app(self):
        ...
        demarcation = Frame(self.aboutWindow, height=1, width=390, highlightthickness=1, highlightbackground='#f2aa4c')
        ...
        demarcation2 = Frame(self.aboutWindow, height=1, width=390, highlightthickness=1, highlightbackground='#f2aa4c')
        ...
        demarcation3 = Frame(self.aboutWindow, height=1, width=390, highlightthickness=1, highlightbackground='#f2aa4c')
    ...
    def add_songs_to_playlist(self, event=None):
        ...
        filenames = askopenfilenames(filetypes=[('audio files', '*.mp3 *.wav *.ogg')])            
    ...
    def show_quit_frame(self, event=None):
        ...
        \n Thank you.
        '''
        the_label = Label(self.quitting_frame,text=about_text,fg='#f2aa4c',bg='#101820',font='montserrat 14')
