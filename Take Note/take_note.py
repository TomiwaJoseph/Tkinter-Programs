...
class App():
    def __init__(self):
        ...
        # ========== Text Area ==========
        self.text_area = Text(autoseparators=True,
                              undo=5, font='montserrat 14')
        ...
        filemenu.add_command(label='New', command=self.new_file,
                             image=self.new_file_icon, accelerator='Ctrl+N', compound='left')
        filemenu.add_command(label='Open', command=self.open_file,
                             image=self.open_file_icon, accelerator='Ctrl+O', compound='left')
        filemenu.add_command(label='Save', command=self.save_file,
                             image=self.save_file_icon, accelerator='Ctrl+S', compound='left')
        ...
        editmenu.add_command(label='Undo', command=self.undo_change,
                             image=self.undo_icon, accelerator='Ctrl+Z', compound='left')
        editmenu.add_command(label='Redo', command=self.redo_change,
                             image=self.redo_icon, accelerator='Ctrl+Y', compound='left')
        editmenu.add_separator()
        editmenu.add_command(label='Cut', command=self.cut_selection,
                             image=self.cut_icon, accelerator='Ctrl+X', compound='left')
        editmenu.add_command(label='Copy', command=self.copy_selection,
                             image=self.copy_icon, accelerator='Ctrl+C', compound='left')
        editmenu.add_command(label='Paste', command=self.paste_selection,
                             image=self.paste_icon, accelerator='Ctrl+V', compound='left')
        editmenu.add_command(
            label='Delete', command=self.delete_selection, accelerator='Del', compound='left')
        editmenu.add_separator()
        editmenu.add_command(
            label='Select All', command=self.select_all_text, accelerator='Ctrl+A', compound='left')
        menubar.add_cascade(label='Edit', menu=editmenu)

        toolmenu = Menu(menubar, tearoff=0)
        toolmenu.add_command(label='Find', command=self.find_text,
                             image=self.find_icon, accelerator='Ctrl+F', compound='left')
        ...
        infomenu.add_command(label='About Developer',
                             command=self.about_developer)
        ...
        for i, icon in enumerate(icons):
            ...
            tool_bar = Button(self.shortcut_bar,
                              image=tool_icon, command=cmd[i])
    ...
    def exit_editor(self):
        ...
        if _file == None:
            if self.text_area.compare("end-1c", "==", "1.0"):
                ...
                confirm_to_quit = messagebox.askyesnocancel(
    ...
    def new_file(self, event=None):
        ...
        if _file == None:
            if self.text_area.compare("end-1c", "==", "1.0"):
                ...
                confirm_to_save = messagebox.askyesnocancel(
                    'Message', "You haven't saved your note. Do you want to save your note?")
                if confirm_to_save:
                    the_note = asksaveasfilename(initialfile='Untitled.txt',
                                                 defaultextension='.txt', filetypes=[('Text Documents', '*.txt')])
                    if the_note == '':
            ...
            if previous_note_content == current_note_content:
                ...
                confirm_to_save = messagebox.askyesnocancel(
                    'Message', "You have made changes to your note. Do you want to save the changes?")
                if confirm_to_save:
    ...
    def find_text(self, event=None):
        ...
        Label(search_toplevel, text="Find All:", font='candara 12').grid(
    ...
    def change_theme(self):
        ...
        self.text_area.config(background=bg_color,
                              fg=fg_color, insertbackground=insertbg)

    def count_text(self):
        text = self.text_area.get(1.0, END)
        messagebox.showinfo(
            'Word Count', f'This note has {len(text.split())} words.')

    def stat_text(self):
        status = stat_bar.get()
        if status:
            self.status_bar.pack(expand='no', fill=None,
    ...
    def about_app(self, event=None):
        ...
        Label(newWindow, text='The app is a basic text editor.',
